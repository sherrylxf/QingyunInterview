# 青云平台 - 云服务器部署文档

## 📋 部署环境信息

- **服务器IP**: 139.199.225.54（公网） / 10.1.0.15（内网）
- **操作系统**: Ubuntu
- **已占用端口**: 80, 443, 5432（PostgreSQL）
- **部署方式**: Docker Compose（容器化部署，便于隔离）

## 🎯 部署架构

```
云服务器
├── 现有项目（80, 443, 5432端口）
└── 青云平台（独立Docker网络）
    ├── Nacos（8848端口）
    ├── MySQL（3306端口，仅内网）
    ├── Redis（6379端口，仅内网）
    ├── Gateway（8080端口）
    ├── Auth（8000端口）
    └── 各业务服务（8001-8005, 9001-9002端口）
```

## 📦 端口分配

| 服务 | 端口 | 说明 |
| ---- | ---- | ---- |
| Nacos | 8848 | 服务注册与配置中心 |
| MySQL | 3306 | 数据库（仅内网访问） |
| Redis | 6379 | 缓存（仅内网访问） |
| Gateway | 8080 | API网关（对外暴露） |
| Auth | 8000 | 认证服务（内网） |
| User Service | 8001 | 用户服务（内网） |
| Question Service | 8002 | 题库服务（内网） |
| Record Service | 8003 | 打卡记录服务（内网） |
| Admin Service | 8004 | 管理员服务（内网） |
| Stat Service | 8005 | 统计分析服务（内网） |
| Web Admin | 9001 | 管理端后端（内网） |
| Web User | 9002 | 用户端后端（内网） |

## 🚀 部署步骤

### 1. 环境准备

```bash
# 更新系统
sudo apt update && sudo apt upgrade -y

# 安装Docker和Docker Compose
sudo apt install -y docker.io docker-compose

# 启动Docker服务
sudo systemctl start docker
sudo systemctl enable docker

# 验证Docker安装
docker --version
docker-compose --version
```

### 2. 创建项目目录

```bash
# 创建项目根目录
sudo mkdir -p /opt/qingyun-platform
cd /opt/qingyun-platform

# 创建必要的目录结构
sudo mkdir -p {logs,data/{mysql,redis,nacos},config}
```

### 3. 上传项目文件

将项目文件上传到服务器（可以使用git clone或scp）：

```bash
# 方式1: 使用Git
cd /opt/qingyun-platform
git clone <your-repo-url> .

# 方式2: 使用SCP（从本地）
scp -r ./qingyun-platform root@139.199.225.54:/opt/
```

### 4. 配置环境变量

创建 `.env` 文件：

```bash
cd /opt/qingyun-platform
sudo nano .env
```

内容如下：

```env
# 数据库配置
MYSQL_ROOT_PASSWORD=123456
MYSQL_DATABASE=qingyun_db
MYSQL_USER=qingyun
MYSQL_PASSWORD=123456

# Redis配置
REDIS_PASSWORD=123456

# Nacos配置
NACOS_SERVER=localhost:8848
NACOS_USERNAME=nacos
NACOS_PASSWORD=123456

# 服务端口配置
GATEWAY_PORT=8080
AUTH_PORT=8000
USER_SERVICE_PORT=8001
QUESTION_SERVICE_PORT=8002
RECORD_SERVICE_PORT=8003
ADMIN_SERVICE_PORT=8004
STAT_SERVICE_PORT=8005
WEB_ADMIN_PORT=9001
WEB_USER_PORT=9002

# JVM参数
JVM_OPTS=-Xms512m -Xmx1024m -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=256m
```

### 5. 构建和启动服务

```bash
# 构建所有服务镜像
docker-compose build

# 启动所有服务（后台运行）
docker-compose up -d

# 查看服务状态
docker-compose ps

# 查看日志
docker-compose logs -f
```

### 6. 初始化数据库

等待MySQL启动后，执行数据库初始化脚本：

```bash
# 进入MySQL容器
docker exec -it qingyun-mysql mysql -uroot -pqingyun123456

# 创建数据库（如果不存在）
CREATE DATABASE IF NOT EXISTS qingyun_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

# 退出MySQL
exit
```

### 7. 配置Nacos

1. 访问Nacos控制台：http://139.199.225.54:8848/nacos
2. 默认账号：nacos / nacos
3. 创建命名空间：`qingyun-dev`
4. 创建公共配置：`qingyun-common.yml`

### 8. 验证部署

```bash
# 检查所有容器状态
docker-compose ps

# 检查服务健康状态
curl http://localhost:8080/actuator/health

# 查看服务日志
docker-compose logs gateway
docker-compose logs auth
```

## 🔧 常用操作命令

### 启动/停止服务

```bash
# 启动所有服务
docker-compose up -d

# 停止所有服务
docker-compose down

# 重启特定服务
docker-compose restart gateway

# 查看服务日志
docker-compose logs -f gateway

# 进入容器
docker exec -it qingyun-gateway bash
```

### 更新服务

```bash
# 重新构建并启动
docker-compose up -d --build

# 更新特定服务
docker-compose up -d --build gateway
```

### 备份数据

```bash
# 备份MySQL数据
docker exec qingyun-mysql mysqldump -uroot -pqingyun123456 qingyun_db > backup_$(date +%Y%m%d).sql

# 备份Redis数据
docker exec qingyun-redis redis-cli SAVE
docker cp qingyun-redis:/data/dump.rdb ./backup_redis_$(date +%Y%m%d).rdb
```

## 🔒 安全配置

### 1. 防火墙配置

```bash
# 开放必要端口
sudo ufw allow 8080/tcp    # Gateway
sudo ufw allow 8848/tcp    # Nacos
sudo ufw allow 22/tcp      # SSH

# 不开放内网服务端口（8000-9002）
# 这些服务只通过Gateway访问
```

### 2. Nginx反向代理（可选）

如果需要通过80/443端口访问，可以配置Nginx反向代理：

```nginx
server {
    listen 80;
    server_name qingyun.yourdomain.com;

    location / {
        proxy_pass http://localhost:8080;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    }
}
```

### 3. 数据库安全

- MySQL仅允许内网访问（Docker网络）
- Redis仅允许内网访问（Docker网络）
- 使用强密码
- 定期备份数据

## 📊 监控和维护

### 查看资源使用

```bash
# 查看容器资源使用
docker stats

# 查看磁盘使用
df -h
du -sh /opt/qingyun-platform/data/*
```

### 日志管理

```bash
# 查看所有服务日志
docker-compose logs --tail=100

# 查看特定服务日志
docker-compose logs -f gateway

# 清理旧日志（保留最近7天）
find /opt/qingyun-platform/logs -name "*.log" -mtime +7 -delete
```

## 🐛 故障排查

### 服务无法启动

```bash
# 查看容器日志
docker-compose logs [service-name]

# 检查端口占用
netstat -tunlp | grep [port]

# 检查Docker网络
docker network ls
docker network inspect qingyun-network
```

### 数据库连接失败

```bash
# 检查MySQL容器状态
docker-compose ps mysql

# 查看MySQL日志
docker-compose logs mysql

# 测试数据库连接
docker exec -it qingyun-mysql mysql -uroot -pqingyun123456 -e "SHOW DATABASES;"
```

### Nacos连接失败

```bash
# 检查Nacos容器状态
docker-compose ps nacos

# 查看Nacos日志
docker-compose logs nacos

# 检查Nacos健康状态
curl http://localhost:8848/nacos/actuator/health
```

## 📝 注意事项

1. **端口隔离**: 所有微服务端口（8000-9002）仅在内网访问，不对外开放
2. **数据持久化**: 重要数据存储在 `/opt/qingyun-platform/data` 目录
3. **日志管理**: 定期清理日志文件，避免磁盘空间不足
4. **备份策略**: 建议每天备份数据库，每周备份完整数据目录
5. **资源监控**: 定期检查服务器资源使用情况，及时扩容

## 🔄 更新部署

当代码更新后，按以下步骤更新部署：

```bash
# 1. 拉取最新代码
cd /opt/qingyun-platform
git pull

# 2. 重新构建镜像
docker-compose build

# 3. 滚动更新服务（先更新后端服务，最后更新网关）
docker-compose up -d --no-deps user-service
docker-compose up -d --no-deps question-service
# ... 其他服务
docker-compose up -d --no-deps gateway

# 4. 验证服务
docker-compose ps
curl http://localhost:8080/actuator/health
```

---

**部署完成后访问地址**:
- API网关: http://139.199.225.54:8080
- Nacos控制台: http://139.199.225.54:8848/nacos
- API文档: http://139.199.225.54:8080/doc.html

